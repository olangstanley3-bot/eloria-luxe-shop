...
import { useStoreProducts, useStoreCategories, useHomepageSettings } from "@/lib/store";
  ...
  head: () => ({
    meta: [
      { title: "Eloria Store · Luxury Curtains, Bedding, Handbags & Shoes in Nairobi" },
      {
        name: "description",
        content:
          "Shop premium curtains, luxury bedding sets, elegant handbags and ladies' shoes in Nairobi, Kenya. Order via WhatsApp +254 742 461 744. Delivery countrywide.",
      },
      {
        property: "og:title",
        content: "Eloria Store · Luxury Curtains, Bedding, Handbags & Shoes in Nairobi",
      },
      {
        property: "og:description",
        content:
          "Premium curtains, luxury bedding, handbags & shoes in Nairobi, Kenya. Order via WhatsApp.",
      },
...
function useCountdown(flashSaleEndTime?: string) {
  ...
  const defaultDuration = 1000 * 60 * 60 * 26;
  const target = flashSaleEndTime ? new Date(flashSaleEndTime).getTime() : 0;

  const diff = mounted
    ? target > 0
      ? Math.max(0, target - now)
      : Math.max(0, defaultDuration - (now % defaultDuration))
    : defaultDuration;
...
function Home() {
  ...
  const products = allProducts.filter((p) => p.isPublished !== false);
  const categories = useStoreCategories();
  const homepage = useHomepageSettings();

  const bestsellers =
    homepage.bestSellerProductIds && homepage.bestSellerProductIds.length > 0
      ? products.filter((p) => homepage.bestSellerProductIds?.includes(p.id))
      : products.filter((p) => p.tag === "Bestseller").length > 0
        ? products.filter((p) => p.tag === "Bestseller").slice(0, 4)
        : products.slice(0, 4);

  const newArrivals =
    homepage.newArrivalProductIds && homepage.newArrivalProductIds.length > 0
      ? products.filter((p) => homepage.newArrivalProductIds?.includes(p.id))
      : products.filter((p) => p.tag === "New" || p.tag === "Limited").length > 0
        ? products.filter((p) => p.tag === "New" || p.tag === "Limited").slice(0, 6)
        : products.slice(0, 6);

  const trending =
    homepage.featuredProductIds && homepage.featuredProductIds.length > 0
      ? products.filter((p) => homepage.featuredProductIds?.includes(p.id))
      : products.slice(0, 8);

  const flash =
    homepage.flashSaleProductIds && homepage.flashSaleProductIds.length > 0
      ? products.filter((p) => homepage.flashSaleProductIds?.includes(p.id))
      ...
      : [
          {
            ...
            name: "Amara K.",
            content:
              "The velvet blackout curtains completely transformed my bedroom. Genuinely luxurious.",
            location: "Nairobi",
          },
          {
            id: "t2",
            name: "Wanjiru M.",
            content:
              "My cream tote receives compliments daily. Better craftsmanship than bags twice the price.",
            location: "Mombasa",
          },
          {
            id: "t3",
            name: "Sasha L.",
            content:
              "The nude pumps are now my favorite pair. Fits perfectly, feels super expensive.",
            location: "Kisumu",
          },
        ];

  const { d, h, m, s } = useCountdown(homepage.flashSaleEndTime);
    ...
    <SiteShell>
          ...
          <div className="relative mx-auto flex h-full max-w-7xl items-center px-6">
            <div className="max-w-xl text-cream animate-fade-up">
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-cream/30 bg-cream/10 px-4 py-1.5 text-xs uppercase tracking-[0.25em] backdrop-blur">
                <Sparkles className="h-3 w-3" /> {homepage.heroBadge}
              </div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link
                  to="/$category"
                  params={{ category: homepage.heroCtaCategory || "handbags" }}
                  className="group inline-flex items-center gap-2 rounded-full bg-cream px-7 py-3.5 text-sm font-medium text-charcoal transition hover:bg-champagne"
                >
                ...
                <a
                  href="#collections"
                  className="rounded-full border border-cream/40 px-7 py-3.5 text-sm font-medium text-cream backdrop-blur hover:bg-cream/10 transition"
                >
                  Browse Collections
                </a>
              </div>
              <div className="mt-10 flex items-center gap-6 text-xs uppercase tracking-widest text-cream/70">
                <div>
                  4.9<span className="ml-1 text-champagne">★</span> · 2,400+ reviews
                </div>
      ...
      <section id="collections" className="mx-auto max-w-7xl px-6 py-20">
        <div className="flex items-end justify-between gap-6">
          ...
          <a href="#trending" className="hidden md:inline text-sm underline underline-offset-4">
            See all
          </a>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <SectionHeader
          eyebrow="The Icons"
          title="Best Sellers"
          link={{ label: "Shop all", to: "/$category", params: { category: "handbags" } }}
        />
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        <div className="rounded-3xl gradient-luxe p-8 md:p-12">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              <div className="text-xs uppercase tracking-[0.3em] text-champagne-deep">
                Flash Deals
              </div>
            ...
            <div className="flex items-center gap-3">
              {[
                ...
                <div
                  key={t.label}
                  className="w-16 rounded-2xl bg-background/80 px-3 py-2 text-center backdrop-blur"
                >
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <SectionHeader
          eyebrow="Just In"
          title="New Arrivals"
          link={{ label: "See more", to: "/$category", params: { category: "shoes" } }}
        />
            ...
            {[
              {
                ...
                body: "Materials and construction we'd stake our name on.",
              },
              {
                icon: Truck,
                title: "Fast Delivery",
                body: "Same-day dispatch in Nairobi. Nationwide in 2–3 days.",
              },
              {
                icon: Undo2,
                title: "Easy Returns",
                body: "14 days, no questions. If it's not right, send it back.",
              },
              {
                icon: HeartHandshake,
                title: "Real Support",
                body: "A team on WhatsApp — not a chatbot in a loop.",
              },
            ].map((f) => (
              <div key={f.title}>
      ...
      <section className="mx-auto max-w-4xl px-6 py-20 text-center">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="mx-auto mt-8 flex max-w-md gap-2">
          ...
          />
          <button className="rounded-full bg-primary px-6 py-3.5 text-sm font-medium text-primary-foreground hover:opacity-90">
            Subscribe
          </button>
        </form>
      </section>
...
function SectionHeader({
  ...
  return (
    <div className="flex items-end justify-between gap-4">
      ...
      {link && (
        <Link
          to={link.to as "/$category"}
          params={link.params as { category: string }}
